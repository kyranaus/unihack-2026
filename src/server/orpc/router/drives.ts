	...
	.handler(async ({ input }) => {
		try {
			...
			if (result.severity !== "info") {
				console.log(
					`[BeeSafe] AI (${input.camera} ${input.elapsedSec}s):`,
					result.severity,
					"-",
					result.summary,
				);
	...
	.handler(async ({ input }) => {
		try {
			if (input.severity !== "info") {
				console.log(
					`[BeeSafe] ${input.type} (${input.camera}): ${input.summary}`,
				);
			}
			const event = await prisma.driveEvent.create({
				data: {
...
export const endSession = os
	...
	.handler(async ({ input }) => {
		...
		console.log(
			`[BeeSafe] Session ended: ${events.length} events, score=${score}, cameras=${cameras.join(",")}`,
		);
		if (summary)
			console.log(`[BeeSafe] AI Summary: ${summary.slice(0, 100)}...`);
...
export const getUploadUrl = os
	.input(z.object({ sessionId: z.string(), mimeType: z.string() }))
	.handler(async ({ input }) => {
		const key = `sessions/${input.sessionId}/video-${Date.now()}.${input.mimeType.includes("mp4") ? "mp4" : "webm"}`;
		const { uploadUrl, videoUrl } = await generatePresignedUploadUrl(
			key,
			input.mimeType,
		);
		await prisma.driveSession.update({
...
export const getProfileStats = os.input(z.object({})).handler(async () => {
	...
	const previousAvgScore =
		sessions.length > 5
			? Math.round(
					sessions.slice(5, 10).reduce((sum, s) => sum + (s.score ?? 0), 0) /
						Math.min(5, sessions.length - 5),
				)
			: avgScore;
