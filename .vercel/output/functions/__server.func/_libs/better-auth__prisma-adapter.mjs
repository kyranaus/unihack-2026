import { c as createAdapterFactory, B as BetterAuthError } from "./better-auth__core.mjs";
const prismaAdapter = (prisma, config) => {
  let lazyOptions = null;
  const createCustomAdapter = (prisma2) => ({ getFieldName, getModelName, getFieldAttributes, getDefaultModelName, schema }) => {
    const db = prisma2;
    ...
    const getJoinKeyName = (baseModel, joinedModel, schema2) => {
      try {
        const defaultBaseModelName = getDefaultModelName(baseModel);
        const defaultJoinedModelName = getDefaultModelName(joinedModel);
        const key = getModelName(joinedModel).toLowerCase();
        let foreignKeys = Object.entries(schema2[defaultJoinedModelName]?.fields || {}).filter(([_field, fieldAttributes]) => fieldAttributes.references && getDefaultModelName(fieldAttributes.references.model) === defaultBaseModelName);
        ...
        foreignKeys = Object.entries(schema2[defaultBaseModelName]?.fields || {}).filter(([_field, fieldAttributes]) => fieldAttributes.references && getDefaultModelName(fieldAttributes.references.model) === defaultJoinedModelName);
    ...
    const convertWhereClause = ({ action, model, where }) => {
      ...
      if (action === "update") {
        ...
        const andSimple = and2.filter((w) => w.operator === "eq" || !w.operator);
        const andComplex = and2.filter((w) => w.operator !== "eq" && w.operator !== void 0);
      ...
      if (action === "delete") {
        ...
        if (idCondition) {
          ...
          const and2 = remainingWhere.filter((w) => w.connector === "AND" || !w.connector);
          const or2 = remainingWhere.filter((w) => w.connector === "OR");
          const andClause2 = and2.map((w) => buildSingleCondition(w));
          const orClause2 = or2.map((w) => buildSingleCondition(w));
          const result = {};
          if (idFieldName in idClause) result[idFieldName] = idClause[idFieldName];
          else Object.assign(result, idClause);
          if (andClause2.length > 0) result.AND = andClause2;
          if (orClause2.length > 0) result.OR = orClause2;
          return result;
        }
      }
    ...
    return {
      ...
      async update({ model, where, update }) {
        ...
        if (!hasRootUniqueWhereCondition(model, where)) {
          const whereClause2 = convertWhereClause({
            model,
            where,
            action: "updateMany"
          });
          ...
          return await db[model].findFirst({ where: whereClause2 });
        }
        const whereClause = convertWhereClause({
          model,
          where,
      ...
      async delete({ model, where }) {
        ...
        if (!where?.some((w) => w.field === "id")) {
          ...
          await db[model].deleteMany({ where: whereClause2 });
          return;
        }
        const whereClause = convertWhereClause({
          model,
